...
<div id="contentCont">
 ...
 <?php
 $category = 'CAT004';
 $query = mysql_query("SELECT * FROM fr_lesson_cat INNER JOIN fr_category ON fr_lesson_cat.id_cat = fr_category.id_cat where fr_category.id_cat='$category'");
 $query2 = mysql_query("SELECT * FROM fr_category where fr_category.id_cat='$category'");
	        ...
	        <td><?php echo $row['lesson_title']; ?></td>
	  		<td align="center">
	  			<form name="ManageGram" method="POST" action="sqls.php">
	  				<input type="hidden" name="idLesson" value="<?php echo $row['id_Lesson']; ?>"></input><input type="hidden" name="idCat" value="<?php echo $row['id_cat']; ?>"></input>
	  				<button name="DelGram" onclick="return confirm('Are you sure you want to delete <?php echo $row['lesson_title']; ?> ?')">Del</button>
	  			</form><button onclick="location.href='articlePost.php?idLesson=<?php echo $row['id_Lesson']; ?>'">Edit</button>
	  		</td>
        </tr>
